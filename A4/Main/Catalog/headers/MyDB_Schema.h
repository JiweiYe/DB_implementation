...
public:
	...
	// get the type of a specific computation, written in exactly the same infix notation used
	// when specifying computations over records
	MyDB_AttTypePtr getType (string ofMe);

	// default constructor
	MyDB_Schema ();

	// constructor taking a list of all of the attributes
	MyDB_Schema (vector <pair <string, MyDB_AttTypePtr>> &allAtts);
