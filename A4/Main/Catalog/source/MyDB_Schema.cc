...
MyDB_AttTypePtr MyDB_Schema :: getType (string compileMe) {
	MyDB_SchemaPtr temp = make_shared <MyDB_Schema> (allAtts);
	MyDB_Record otherTemp;
	otherTemp.mySchema = temp;
	otherTemp.buffer = new char[1];
	char *str = (char *) compileMe.c_str ();
	return otherTemp.compileHelper (str).second;
}

MyDB_Schema :: MyDB_Schema () {}

MyDB_Schema :: MyDB_Schema (vector <pair <string, MyDB_AttTypePtr>> &allAttsIn) {
	allAtts = allAttsIn;
}
